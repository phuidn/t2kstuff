		...
		for (int j=0; j<NPIDs; j++) {
			...
			if(ABS(vec.X())<832.2 && ABS(vec.Y()-55)<832.2 && ((vec.Z()>123.45&&vec.Z()<446.95)||(vec.Z()>1481.45&&vec.Z()<1807.95))){	//is it in one of the FGDs?
				...
				if(vtx.ReactionCode.find("Weak[NC],QES;",0)!=-1){
					nfgdqes++;
					if( vtx.ReactionCode.find("N:2112",0)!=-1)
						fgdqesneu++;	
				}	
			}
...
//plotting bits at the end :D
	Double_t fgdratio = (Double_t)nfgd/(Double_t)ninteract;
	Double_t fgdqesratio = (Double_t)nfgdqes/(Double_t)ninteract;
	Double_t fgdqesneuratio = (Double_t)fgdqesneu/(Double_t)ninteract;
	cout << "ratio of interactions in the FGD = " << fgdratio << endl;
	cout << "ratio of QES neutron interactions in the FGD = " << fgdqesratio << endl;
	cout << "ratio of QES neutron interactions off neutrons in the FGD = " << fgdqesneuratio << endl;
	return 0;
}
